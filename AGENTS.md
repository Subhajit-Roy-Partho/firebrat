...
- Pipeline entrypoint: `backend/convert.py` → `backend/firebrat/pipeline/*.py` (extract/extract_batch_cli → compile_llm → formulas/tts/audio_assemble → manifest). It now writes live progress to each package's `status.json` via `firebrat/pipeline/status.py` — read that before adding a new stage or you'll leave the job queue blind to it.
- Shared schema (used by extraction, LLM validation, manifest building, and API responses): `backend/firebrat/pipeline/schema.py`.
- API contract: `backend/server/routes/books.py` (read-only package serving) + `backend/server/routes/jobs.py` (upload/status/retry, backed by `server/jobs.py` SQLite table + `server/job_runner.py`'s subprocess queue), must match `docs/API.md` and `docs/DATA_SCHEMA.md` exactly. **Gotcha**: every module that reads `server.config` values must do `from server import config` and reference `config.X` at call time, not `from server.config import X` at module top — the latter freezes the value at first import, which silently breaks test monkeypatching (and would break real runtime env-var overrides too, if config were ever mutated post-startup). `server/storage.py`, `server/jobs.py`, `server/job_runner.py`, `server/routes/jobs.py` all follow the call-time pattern; don't reintroduce the frozen-import one.
- Reader sync logic (the actual "highlight while it's spoken" feature): `frontend/firebrat_app/lib/services/playback_controller.dart` (drives active-segment state from position stream, now backed by the app-wide `FirebratAudioHandler` singleton in `lib/services/audio_handler.dart` rather than owning its own player — see that file's docstring before assuming a `PlaybackController` owns/disposes the underlying `AudioPlayer`) and `lib/state/reader_providers.dart` (owns the section/playback session, autoplay logic, current-visual tracking for Focus Mode + lock-screen art).
- Docker images: `backend/docker/` (Dockerfile = all-in-one CPU/GPU via `--build-arg TORCH_INDEX_URL`, `Dockerfile.api` = thin server-only). **Never built or run in this sandbox** — no Docker, no rootless-build tooling, no subuid/subgid for user namespaces. `.github/workflows/docker-publish.yml` is the actual build/push path (GitHub-hosted runners have Docker). Treat these Dockerfiles as reviewed-but-unverified until a real build succeeds somewhere.
